...
static void init_tables(void)
{
	...
	for (x=0; x<4; x++)
	{
		...
		if (m>0.0)
			o = 8*log(255.0/m)/log(2.0);  /* convert to 'decibels' */
		else
		{
			if (m<0.0)
				o = 8*log(-255.0/m)/log(2.0); /* convert to 'decibels' */
			else
				o = 8*log(255.0/0.01)/log(2.0); /* small number */
		}
	...
	for (i=0; i<128; i++)
	{
		...
		if (m>0.0)
			o = 8*log(8192.0/m)/log(2.0);  /* convert to 'decibels' */
		else
			o = 8*log(8192.0/0.01)/log(2.0); /* small number (m=0)*/
	...
	for (x=0; x<4; x++)
	{
		...
		if (m>0.0)
			o = 8*log(127.0/m)/log(2.0);  /* convert to 'decibels' */
		else
		{
			if (m<0.0)
				o = 8*log(-128.0/m)/log(2.0); /* convert to 'decibels' */
			else
				o = 8*log(127.0/0.01)/log(2.0); /* small number */
		}
...
static void init_chip_tables(YM2151 *chip)
{
	...
	for (i=0; i<1*12*64; i++)
	{
		...
		Hz = scaler * 3.4375 * pow (2.0, (i+4*64) / 768.0 );
